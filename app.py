    ...
    if total_h >= 1:
        ...
        if total_h >= 10:
            ...
            if abs(avg_val - 7) > 1.5:
                st.warning(f"⚠️ 偏離警戒: 重心偏向 {'大' if avg_val > 7 else '小'}號區")
...
if st.session_state.history:
    ...
    if curr_len > 0 and curr_len % 36 == 0:
        st.info(f"💡 **週期提醒**: 已記錄 {curr_len} 手數據。建議考慮清空數據重置分析週期。")
    ...
    # 冷門避雷
    bot_list = df_res.tail(2)['數字'].astype(int).tolist()
    st.write(f"❄️ 冷門避雷: `{bot_list[0]}`, `{bot_list[1]}`")
